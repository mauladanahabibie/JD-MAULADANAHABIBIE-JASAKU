<div>
    ...
    <div id="chatModal"
        ...
        <div class="bg-white rounded-lg w-full max-w-4xl max-h-[90vh] flex flex-col md:flex-row shadow-lg md:shadow-2xl">
                    ...
                    @if ($users)
                        @foreach ($users as $user)
                            <li wire:click="selectUser({{ $user->id }})"
                                class="p-2 rounded cursor-pointer {{ $selectedUser->id === $user->id ? 'bg-tertiary/10' : '' }}" @click.stop>
                                {{ $user->name }}
                            </li>
                        @endforeach
                    @endif
                            ...
                            @if ($message->sender_id === auth()->id())
                                <div class="flex justify-end">
                                    <div class="bg-tertiary text-white rounded-lg p-3 max-w-xs md:max-w-md">
                                        ...
                                        <span
                                            class="text-xs opacity-80 mt-1 block text-right">{{ $message->created_at->format('H:i') }}</span>
                                    </div>
                                </div>
                            @else
                                <div class="flex justify-start">
                                    <div class="bg-white border border-gray-300 rounded-lg p-3 max-w-xs md:max-w-md">
                                        <p class="text-sm">{{ $message->message }}</p>
                                        <span
                                            class="text-xs text-gray-500 mt-1 block">{{ $message->created_at->format('H:i') }}</span>
                                    </div>
                                </div>
                            @endif
                        @endforeach
